...
#TODO (INFORMAL)
vertices = [
    [args.start_pos_x, args.start_pos_y, args.start_pos_theta],
...
def getRobotSegment(point):
    ...
    end1 = [point[0] + extend[0], point[1] + extend[1]]
    end2 = [point[0] - extend[0], point[1] - extend[1]]
...
def returnParent(k, canvas, G):
    ...
    for e in G[edges]:
        ...
        if e[1]==k:
            prev = vertices[e[0]]
            robot_end1, robot_end2 = getRobotSegment(prev)
            canvas.polyline([robot_end1, robot_end2], style=4)
...
#TODO (INFORMAL)
def lineFromPoints3d(p1,p2): # vector starts from p1
...
def rrt_search(G, tx, ty, tz, canvas):
    #TODO
    ...
    while 1:
        ...
        vector = lineFromPoints3d(vertices[v], p)
        ...
        step_vector = [
            SMALLSTEP * unit_vector[0],
            SMALLSTEP * unit_vector[1],
            SMALLSTEP * unit_vector[2],
        ]
        ...
        for o in obstacles:
            #if inRect(p,o,1):
            # print "TODO"
            robot_end1, robot_end2 = getRobotSegment(p)
            if lineHitsRect(robot_end1, robot_end2, o) \
            or inRect(robot_end1, o, 1) \
            or inRect(robot_end2, o, 1):
            # or lineHitsRect(vertices[v],p,o):
                restart = True
                break
                #... reject
